...
struct DelayerAgent{T <: AbstractFloat, U <: Unsigned, V <: Signal}
    delay::T # delay time
    w::AbstractFloat
    stack::Vector{Tuple{T, V}} # (time left, signal)
    donor::Address{U}
    acceptor::Address{U}
end
...
function DelayerAgent(
    ...
    if donor_check && acceptor_check
        DelayerAgent{T, U, V}(delay, w, [], donor, acceptor)
    else
...
function update(agent::DelayerAgent{T, U, V},
                updates::Vector{Tuple{T, V}}) where {T <: AbstractFloat, U <: Signal, V <: ForwardSignal}
    agent.stack = updates
end
...
function act(address::Address{U},
    ...
    new_stack, due_stack = reduce(
        agent.stack;
        Init=(Vector{(T, V)}(undef, 0), Vector{V}(undef, 0))
    ) do (acc, pair)
        new_t = pair[1] - dt
        if new_t >= 0

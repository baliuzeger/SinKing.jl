module SinKing

mutable struct IZStates{T <: AbstractFloat}
    iz_u::T
    iz_v::T
    g_ampa::T
    g_nmda::T
    g_gaba_a::T
    g_gaba_b::T
    exct_mrkrm_r::T
    exct_mrkrm_w::T
    inhbt_mrkrm_r::T
    inhbt_mrkrm_w::T
    idle_end::Union{Nothing, T}
end

struct IZParams{T <: AbstractFloat}
    iz_a::T
    iz_b::T
    iz_c::T
    iz_d::T
    tau_refraction::T
    tau_ampa::T
    tau_nmda::T
    tau_gaba_a::T
    tau_gaba_b::T
    exct_mrkrm_d::T
    exct_mrkrm_f::T
    exct_mrkrm_u::T
    inhbt_mrkrm_d::T
    inhbt_mrkrm_f::T
    inhbt_mrkrm_u::T
end

abstract type Donor end # T::generated by agent, to be processed for sending
abstract type Acceptor end # T::processed as what agent accepts

struct TimedTransmitter
    time
    dosage
end

struct IZNeuron{T <: AbstractFloat}
    states::IZStates{T}
    params::IZParams{T}
    donors::Vector{Donor}
    acceptors::Vector{Acceptor}
end

struct LIFNeuron
end

function evolve(agent::IZNeuron, dt, modifier)
    
end

# accept(acceptor) return [] of msgs
function act(agent::IZNeuron, t, dt, modifier)
    if ! isnothing(agent.states.idle_end) && agent.states.idle_end <= t
        delta_exct, delta_inhbt, delta_potential = reduce(
            acc, x -> (acc[1] + x[1], acc[2] + x[2], acc[3] + x[3]),
            vcat(accept(agent.acceptors)),
            (0., 0., 0.,)
        )
        
        modifier(agent, IZStates(agent.states.iz_u,
                                 agent.states.iz_v,
                                 agent.states.g_ampa * (1 - dt / agent.params.tau_ampa) + delta_exct,
                                 agent.states.g_nmda * (1 - dt / agent.params.tau_nmda) + delta_exct,
                                 agent.states.g_gaba_a * (1 - dt / agent.params.tau_gaba_a) + delta_inhbt,
                                 agent.states.g_gaba_b * (1 - dt / agent.params.tau_gaba_b) + delta_inhbt,
                                 agent.states.exct_mrkrm_r,
                                 agent.states.exct_mrkrm_w,
                                 agent.states.inhbt_mrkram_r,
                                 agent.states.inhbt_mrkram_w,
                                 nothing)

        stts = agent.states
        prms = agent.params
        i_syn = stts.g_ampa(stts.iz_v) +
            stts.g_nmda(stts.iz_v)((stts.iz_v + 80) / 60)^2 / (1 + ((stts.iz_v + 80) / 60)^2) +
            stts.g_gaba_a(stts.iz_v + 70) + stts.g_gaba_b(stts.iz_v + 90)
        new_v = stts.iz_v + dt * (0.04 stts.iz_v^2 + 5 stts.iz_v + 140 - stts.iz_u - i_syn)
        new_u = stts.iz_u + dt * prms.iz_a (prms.iz_b * stts.iz_v - stts.iz_u)
        if new_v >= 30
            new_v = prms.iz_c
            new_u += prms.iz_d
            rw_exct = stts.exct_mrkrm_r * stts.exct_mrkrm_w
                 rw_inhbt = stts.inhbt_mrkrm_r * stts.inhbt_mrkrm_w
                 modifier(agent, IZStates(new_u,
                                          new_v,
                                          agent.states.g_ampa,
                                          agent.states.g_nmdap,
                                          agent.states.g_gaba_a,
                                          agent.states.g_gaba_b,
                                          agent.states.exct_mrkrm_r xxxx,
                                          agent.states.exct_mrkrm_w xxx,
                                          agent.states.inhbt_mrkram_r sss,
                                          agent.states.inhbt_mrkram_w sss,
                                          t + tau_refraction))
            for dnr in agent.donors
                dnr.
                    end
        end
                         
end


end # module
